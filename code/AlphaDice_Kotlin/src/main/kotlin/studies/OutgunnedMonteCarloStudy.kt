...
class OutgunnedMonteCarloStudy(_iter: Int, _dice: Int) : MonteCarloStudy(_iter) {

    companion object {
        fun multiRun() {
            // rerun with dice pools from 0 to 9
            (0 until (10)).forEach { OutgunnedMonteCarloStudy(1000000, it).run() }
        }
    }

    var dice = _dice
    override var name = "Outgunned Study (Monte Carlo): ${dice}d"
...
//    override var outcomeComparator = Comparator<Outcome> {o1, o2 -> (o1.name).compareTo(o2.name) }
    ...
    override fun init() {
        ...
        (0 until diceNum).forEach { pool.dice.add(Die("die-$it",6)) }
    ...
    private fun gatherSets() {
        (0..6).forEach { sets[it] = 0 }
        pool.dice.forEach { sets[it.get()] = min(sets[it.get()] + 1, 6) }
        ones = sets[1]

        largestSet = 0
        (lowend..6).forEach { if (sets[it] >= sets[largestSet]) largestSet = it }

        successes = 0
        (lowend..6).forEach {
            if (sets[it] > 1) {
                successes += min(81, ((3.0).pow(sets[it]-2)).toInt())
            }
        }
    }
