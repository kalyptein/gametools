
## Basic Rules

d20 Rolls

Outcome
* Critical success: natural 20 or rolling >= DC+10.
* Critical failure: natural 1 or rolling <= DC-10

DCs
* Easy: 9
* Normal: 12
* Hard: 15
* Extreme: 18

Modifiers
* Advantage
* Disadvantage
* cancel and stack

Pushing
* reroll the check, 
* logical consequences, fumble outcome on fail, suffer d4 stat damage?
* can't use Luck after Pushing, must use it before

Training
* A character who is trained in a field of endeavor (e.g. stealth, diplomacy, etc) gets advantage on related checks.

Momentum
* if you keep working at a task that can be retried, take +A on subsequent rounds

Piggybacking

Group Checks



* Rolls
	* Natural 1 always fails; natural 20 always succeeds
	* Add an appropriate MOD to your save roll.  Use the single best value if multiple options apply.


* if you have +A on certain kinds of actions, you don't need to roll.  if countered by +D, this ceases to apply
	* e.g. with +A to the climb (from training, gear, etc), rough climbing at half speed or less doesn't require a check.  Moving faster imposes +D, rain might impose +D, etc.


## Luck

* spend a luck to turn a normal hit into a crit
* start with 1 luck when you set forth fresh from a respite (town, etc)
* gain from a heroic deed or sacrifice
* when you long rest (or maybe when you respite and/or at end of session?), if you have >1 Luck Token, drop to 1 luck token

inspiration tokens
* last until you Long Rest
	* granted by bards, Bless, etc.
* otherwise function as Luck tokens

## Time & Distance

Distance
* Close: about 5'
* Reach: about 10-15'
* Near: about 30'
* Far: 4x Near, about 120'



